...
HmObjectTracker::HmObjectTracker(const rclcpp::Node::SharedPtr & node) {
  // Initialize tracker's configs
  config_.name = read_parameter(node, "name", std::string("HmObjectTracker"));
  config_.version = read_parameter(node, "version", std::string("1.1.0"));
  config_.matcher_method = read_parameter(node, "matcher_method", std::string("HUNGARIAN_MATCHER"));
  config_.filter_method = read_parameter(node, "filter_method", std::string("KALMAN_FILTER"));
  config_.track_cached_history_size_maximum = read_parameter(node, "track_cached_history_size_maximum", 5);
  config_.track_consecutive_invisible_maximum = read_parameter(node, "track_consecutive_invisible_maximum", 2);
  config_.track_visible_ratio_minimum = read_parameter(node, "track_visible_ratio_minimum", 0.6);
  config_.collect_age_minimum = read_parameter(node, "collect_age_minimum", 0);
  config_.collect_consecutive_invisible_maximum = read_parameter(node, "collect_consecutive_invisible_maximum", 0);
  config_.acceleration_noise_maximum = read_parameter(node, "acceleration_noise_maximum", 5);
  config_.speed_noise_maximum = read_parameter(node, "speed_noise_maximum", 0.4);
  config_.match_distance_maximum = read_parameter(node, "match_distance_maximum", 4.0);
  config_.location_distance_weight = read_parameter(node, "location_distance_weight", 0.6);
  config_.direction_distance_weight = read_parameter(node, "direction_distance_weight", 0.2);
  config_.bbox_size_distance_weight = read_parameter(node, "bbox_size_distance_weight", 0.1);
  config_.point_num_distance_weight = read_parameter(node, "point_num_distance_weight", 0.1);
  config_.histogram_distance_weight = read_parameter(node, "histogram_distance_weight", 0.5);
  config_.histogram_bin_size = read_parameter(node, "histogram_bin_size", 10);
  config_.use_adaptive = read_parameter(node, "use_adaptive", true);
  config_.measurement_noise = read_parameter(node, "measurement_noise", 0.4);
  config_.initial_velocity_noise = read_parameter(node, "initial_velocity_noise", 5);
  config_.xy_propagation_noise = read_parameter(node, "xy_propagation_noise", 10);
  config_.z_propagation_noise = read_parameter(node, "z_propagation_noise", 10);
  config_.breakdown_threshold_maximum = read_parameter(node, "breakdown_threshold_maximum", 10.0);
