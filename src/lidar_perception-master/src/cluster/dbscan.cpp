#include "dbscan.h"
namespace cluster{
dbscan::dbscan(const rclcpp::Node::SharedPtr & node)
{
  min_sample_ = node->declare_parameter("min_sample", 3);
  eps_ = node->declare_parameter("eps", 10);
  pic_height_ = node->declare_parameter("pic_height", 400);
  pic_width_ = node->declare_parameter("pic_width", 1200);
